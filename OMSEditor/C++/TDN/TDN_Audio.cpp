...
//
//**************************************************************************************************************
tdnSoundBuffer::tdnSoundBuffer(LPDIRECTSOUND8 lpDS, LPCSTR lpcFileName, bool b3D)
{
	/*	サウンドファイルのロード	*/
	lpWBuf = LoadFile(lpcFileName, &size, &wfx);
	/*	ロード失敗	*/
	if (lpWBuf == nullptr){
		return;
	}

	//sprintf_s(wav_file_path, sizeof(wav_file_path), "%s", lpcFileName);
...
void tdnSoundBuffer::Create_and_copy(LPDIRECTSOUND8 lpDS, LPCSTR lpcFileName, bool b3D, tdnSoundBuffer **buffers, int dst, int count)
{
	DSBUFFERDESC	dsbd;
	LPVOID			lpbuf1, lpbuf2;
	DWORD			dwbuf1, dwbuf2;
	...
	/*	WAVファイルのロード	*/
	buffers[dst]->lpWBuf = buffers[dst]->LoadFile(lpcFileName, &buffers[dst]->size, &buffers[dst]->wfx);

	/* ファイル名保存 */
	//sprintf_s(buffers[dst]->wav_file_path, sizeof(buffers[dst]->wav_file_path), "%s", lpcFileName);
...
//		ＷＡＶファイルの読み込み
//**************************************************************************************************************
LPBYTE tdnSoundBuffer::LoadFile(LPCSTR lpcFileName, LPDWORD size, LPWAVEFORMATEX wfx)
{
	//	ファイル読み込み
	std::string e = tdnFile::GetFileExtention(lpcFileName);
	char* ext = (char*)e.c_str();
	if (lstrcmpi(ext, ".wav") == 0) return LoadWAV(lpcFileName, size, wfx);
	else if (lstrcmpi(ext, ".owd") == 0) return LoadOWD(lpcFileName, size, wfx);
	else if (lstrcmpi(ext, ".mp3") == 0) return LoadMP3(lpcFileName, size, wfx);
	else if (lstrcmpi(ext, ".ogg") == 0) return LoadOGG(lpcFileName, size, wfx);
	else
	{
		MyAssert(0, "ファイルオープンに失敗！\nファイル名: %s", lpcFileName);
		return nullptr;
	}
}

LPBYTE tdnSoundBuffer::LoadWAV(LPCSTR lpcFileName, LPDWORD size, LPWAVEFORMATEX wfx)
{
	// バイナリ読み込み
	std::ifstream infs(lpcFileName, std::ios::binary);
	MyAssert(infs, "エラーファイル名[%s]\n原因:wavファイルが入っていないか、wavファイル名が間違っているよ", lpcFileName);
	...
	MyAssert(
		...
		chunkID[1] == 'm'&&
		chunkID[2] == 't',
		"エラーファイル名[%s]\n原因:フォーマットチャンク内でエラー", lpcFileName);
	...
	MyAssert(
		...
		chunkID[2] == 't'&&
		chunkID[3] == 'a',
		"エラーファイル名[%s]\n原因:データチャンク内でエラー。フォーマットのずれが原因かもしれない", lpcFileName);
...
LPBYTE tdnSoundBuffer::LoadOWD(LPCSTR lpcFileName, LPDWORD size, LPWAVEFORMATEX wfx)
{
	// バイナリ読み込み
	std::ifstream infs(lpcFileName, std::ios::binary);
...
//	MP3
//-------------------------------------------------------------
LPBYTE tdnSoundBuffer::LoadMP3(LPCSTR lpcFileName, LPDWORD size, LPWAVEFORMATEX wfx)
{
	// バイナリ読み込み
	std::ifstream infs(lpcFileName, std::ios::binary);
	MyAssert(infs, "エラーファイル名[%s]\n原因:MP3ファイル名が間違っているか、ファイルが存在しないよ", lpcFileName);
	...
	WORD frame_synchronism = (receive >> 20) & 0x000007ff;	// フレーム同期。オール1ビット(11ビット)
	MyAssert(frame_synchronism == 0x000007ff, "エラーファイル名[%s]\n原因:形式はMP3ですが、中身がMP3でない可能性があります", lpcFileName);
...
//	OGG
//-------------------------------------------------------------
LPBYTE tdnSoundBuffer::LoadOGG(LPCSTR lpcFileName, LPDWORD size, LPWAVEFORMATEX wfx)
{
	LPBYTE buf{};
	OggVorbis_File vf{};
	FILE *fp{};
	auto Close=[](FILE *fp, OggVorbis_File *vf)
	{
		// OggVorbisファイルを閉じる
		if (fp) fclose(fp);
		ov_clear(vf);
	...
	//	ファイルオープン 
	MyAssert(fopen_s(&fp, lpcFileName, "rb") == S_OK, "ファイルオープンに失敗！\nファイル名: %s", lpcFileName);
	...
	MyAssert(vi, "ov_info関数に失敗");

	// WAVEフォーマット設定
	wfx->wFormatTag = WAVE_FORMAT_PCM;
	wfx->nChannels = vi->channels;
	wfx->nSamplesPerSec = vi->rate;
	wfx->nAvgBytesPerSec = vi->rate * vi->channels * 2;
	wfx->nBlockAlign = vi->channels * 2;
	wfx->wBitsPerSample = 16;
	wfx->cbSize = sizeof(WAVEFORMATEX);

	// サイズ取得
	*size = (DWORD)ov_pcm_total(&vf, -1) * wfx->nBlockAlign;

	// 再生用バッファの作成
	buf = (LPBYTE)GlobalAlloc(LPTR, *size);

	DWORD l_dwTotalReadSize{};
	int l_iBitStream{};

	while (l_dwTotalReadSize < *size)
	{
		// データのデコード
		LONG readSize = ov_read(
			&vf,
			(char*)buf + l_dwTotalReadSize,
			*size - l_dwTotalReadSize,
			0,
			2,
			1,
			&l_iBitStream
...
void tdnSoundBuffer::PlayOffset(bool loop, DWORD msOffset)
{
	lpBuf->Stop();
	DWORD set = msOffset * (int)(format.nAvgBytesPerSec * .001f);
	HRESULT hr = lpBuf->SetCurrentPosition(set);

	//tdnDebug::OutPutLog("%x, %d, %d, %d", hr, set, msOffset, format.nAvgBytesPerSec);
...
DWORD tdnStreamSound::m_PlayMSecond = 0;
...
DWORD WINAPI ThreadIIDX(LPDWORD lpdwParam)
{
	...
	for (;;){
		...
		switch (param){
		case WAIT_OBJECT_0:
			if (lpStream->type == TYPE_WAV) lpStream->SetBlockWAV(1);
			if (lpStream->type == TYPE_OGG) lpStream->SetBlockOGG(1);
			break;
		case WAIT_OBJECT_0 + 1:	
			//tdnStreamSound::m_PlayMSecond += lpStream->GetBlockMSecond();
			if (lpStream->type == TYPE_WAV) lpStream->SetBlockWAV(0);
		...
		case STR_FADEOUT:	lpStream->SetVolume(lpStream->GetVolume() - lpStream->GetParam());
			if (lpStream->GetVolume() <= 0){
				lpStream->Stop();
				delete lpStream, lpStream = nullptr;
				ExitThread(TRUE);
				return 0;
			}
			break;
...
//
//**************************************************************************************************************
tdnStreamSound::tdnStreamSound(LPDIRECTSOUND lpDS, LPSTR filename, BYTE mode, int param, bool bLoop) :m_bLoop(bLoop), mode(mode), param(param), m_bEnd(false)
{
	m_PlayMSecond = 0;
	NumStream++;
...
BOOL	tdnStreamSound::SetBlockOGG(int block)
{
	...
	//	ブロック１へのデータセット
	bEnd = !OGGRead(blk1, bs1);
	StrPos += bs1;
	//	ブロック２へのデータセット
	if (blk2){
		(bEnd)? bEnd = !OGGRead(blk2, bs2) : OGGRead(blk2, bs2);
		StrPos += bs2;
	}
...
void tdnStreamSound::Initialize(LPDIRECTSOUND lpDS, int rate)
{
	...
	wfx.nBlockAlign = 4;
	wfx.nAvgBytesPerSec = m_AvgBytesPerSec = rate * 4;
	/* 二次バッファの初期化	*/
	DSBUFFERDESC dsbd{};
	dsbd.dwSize = sizeof(DSBUFFERDESC);
...
//=============================================================================================
//		waveファイルセット
void tdnSoundSE::Set(int ID, int num_of_play_simultaneously, LPCSTR lpcFileName, bool b3D)
{
	//	初期化チェック
	assert(lpDS);
	//	既存のバッファの解放
	...
	{	// 1個しかないならコピーする意味ないじゃない！
		...
		set->buffer = new tdnSoundBuffer(lpDS, lpcFileName, b3D);
		set->b3D = b3D;
	...
	else
	{
		...
		tdnSoundBuffer::Create_and_copy(lpDS, lpcFileName, b3D, work, 0, num_of_play_simultaneously);	// 上でworkにアドレスを渡したのでここでもうbuffer[ID]には作られている
		delete[] work;
...
//=============================================================================================
//		waveファイルセット
void tdnSoundBGM::Set(int ID, LPCSTR lpcFileName, bool b3D)
{
	//	初期化チェック
	assert(lpDS);
	...
	if (ID < (int)data.size())
	{
		SAFE_DELETE(data[ID]->buffer);

		data[ID]->b3D = b3D;
		data[ID]->fade_mode = MODE::NONE;
		data[ID]->fade_speed = 0;
		data[ID]->volume = 1;
		data[ID]->buffer = new tdnSoundBuffer(lpDS, lpcFileName, b3D);
...
void tdnSoundBGM::PlayOffset(int ID, bool loop, DWORD msOffset)
{
	//	初期化チェック
	assert(lpDS);
	//	データが無い！！
	assert(data[ID]->buffer);
	data[ID]->buffer->PlayOffset(loop, msOffset);
}
...
void tdnSoundBGM::PauseOff(int ID, float FadeSpeed)
{
	assert(lpDS);
	assert(data[ID]->buffer);
	...
	if (FadeSpeed < 1)
	{
		...
		data[ID]->fade_speed = FadeSpeed;
		data[ID]->buffer->SetVolume(DSBVOLUME_MIN);
	}
	data[ID]->buffer->PauseOff();
}
//
//=============================================================================================
...
//	ストリームサウンド管理
//**************************************************************************************************************
tdnStreamSound* tdnSoundBGM::PlayStream(char* filename, bool bLoop)
{
	return PlayStream(filename, STR_NORMAL, 0, bLoop);
}

tdnStreamSound* tdnSoundBGM::PlayStream(char* filename, BYTE mode, int param, bool bLoop)
{
	// ファイル名あってるか確認
	FILE *fp;
	fopen_s(&fp, filename, "rb");
	MyAssert(fp, "tdnStreamSound: ファイル名間違えてるよ！");
	fclose(fp);
	...
	lpStream = new tdnStreamSound(lpDS, filename, mode, param, bLoop);
	return lpStream;
}
...
{
	// ファイル拡張子取得
	std::string e = tdnFile::GetFileExtention(ReadPath);
	char* ext = (char*)e.c_str();
		
	BYTE ver;		// OWDバージョン
	OWDInfo OWDinfo;// OWD情報
	ZeroMemory(&OWDinfo, sizeof(OWDInfo));

	// 拡張子でバージョン
	if (lstrcmpi(ext, ".wav") == 0) ver = 0;
	else if (lstrcmpi(ext, ".ogg") == 0) ver = 1;
	else  throw std::runtime_error("OWD: 対応していないファイル");

	// WAVファイルそのまま書き出しバージョン
	if (ver == 0)
	{
		// データ取得
		OWDinfo.data = LoadWavData(ReadPath, &OWDinfo.size, &OWDinfo.wfx);
		...
		ofs.write((char*)&OWDinfo.wfx, sizeof(WAVEFORMATEX));
		ofs.write((char*)&OWDinfo.size, 4);
		int remain = OWDinfo.size; // 書き込むべき残りのバイト数
		for (int i = 0; remain > 0; i++)
		{
			// 1024Bytesずつ書き込む(メモリパンクさせないように)
			int writeSize = min(1024, remain);
			ofs.write((char*)&OWDinfo.data[i * 1024], sizeof(char)* writeSize);
			remain -= writeSize;
		}
	...
	else if (ver == 1)
	{
		...
		OWDinfo.wfx.wFormatTag = WAVE_FORMAT_PCM;
		OWDinfo.wfx.nChannels = static_cast<WORD>(info->channels);
		OWDinfo.wfx.nSamplesPerSec = info->rate;
		OWDinfo.wfx.wBitsPerSample = 16;
		OWDinfo.wfx.nBlockAlign = static_cast<WORD>(info->channels * 2);
		OWDinfo.wfx.nAvgBytesPerSec = OWDinfo.wfx.nSamplesPerSec * OWDinfo.wfx.nBlockAlign;
	}
}