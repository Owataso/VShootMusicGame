...
//	コンストラクタ
//------------------------------------------------------
iexMesh::iexMesh(LPCSTR filename)
{
	lpMesh = NULL;
	lpTexture = NULL;
	lpNormal = NULL;
	...
	//	ファイル読み込み
	std::string ext = "";
	for (int i = lstrlen(filename) - 4; filename[i] != '\0'; i++)
	{
		ext += filename[i];
	}

	if (lstrcmpi(ext.c_str(), ".imo") == 0) LoadIMO(filename);
	else LoadX(filename);
...
//	ＩＭＯ読み込み
//------------------------------------------------------
BOOL iexMesh::LoadIMO(LPCSTR filename)
{
	IMOOBJ		imo;
	DWORD		i;
	//	ファイル読み込み
...
//	Ｘファイル読み込み
//------------------------------------------------------
BOOL	iexMesh::LoadX(LPCSTR filename)
{
	HRESULT hr;
